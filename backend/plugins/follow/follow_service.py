...
class FollowService(Plugin):
    ...
    def _on_start(self):
        ...
        try:
            ...
            print("[FollowService] Started with DirectStrategy, expo=0")
        except Exception as e:
            print(f"[FollowService] Warning: {e}")
    ...
    def _loop(self):
        ...
        for frame in self.frames:
            ...
            self.send_overlay(json.dumps(overlay))
            ...
            if now - last_log_time >= self.log_interval:
                ...
                print(f"[FollowService] offset: {center_offset:+.1f}%, box: {box_w/frame_w*100:.1f}%, yaw={yaw:.0f}, pitch={pitch:.0f}")
                last_log_time = now
